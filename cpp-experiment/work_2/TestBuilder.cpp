...
void TestBuilder::AddWord()
{
	...
	cin >> s;
	unsigned int i = 0;
	for (; i < Game::getWordList().size(); i++){//找到属于哪个难度
		if (s.size() < Game::getWordList()[i][0].size())
			break;
	}
	if (Game::getWordList()[i - 1][0].size() == s.size()) {//长度相同
		for (unsigned int m = 0; m < Game::getWordList()[i - 1].size(); m++) {//遍历该难度中的单词，若重复则报错
			if (Game::getWordList()[i - 1][m] == s) {
				cout << "The word is already exited! Add word failed!" << endl;
				return;
			}
		}
		Game::getWordList()[i - 1].push_back(s);//没报错则加入这个单词(在待使用的单词表)
		//将该单词添加到文件wordList.csv中【栈的思想】
		string n;
		vector<string> s1;//长度大于等于s的单词
		vector<string> s2;//长度小于s的单词
	...
	else if (i < Game::getWordList().size()) {//单词表中不存在此难度的单词则建立新的难度
		vector <string> add;
		add.push_back(s);
